"""
Motion controller for waypoint following using inverse kinematics.
Uses PID controllers for heading and distance control with wheel encoder models.
"""
...
from python_simulations.robot_models import (
    PidControl,
    WheelWithEncoder,
...
class MotionController:
    ...
    def __init__(self):
        ...
        # Waypoint queue
        self.waypoints = []
        self.original_waypoints = []  # Store original waypoints for looping
    ...
    def add_waypoint(self, x: float, y: float):
        ...
        self.original_waypoints.append(wp)
        
    def update(self, pos_x: float, pos_y: float, heading: float, dt: float = 0.025) -> tuple:
        """
        Update motion control using PyBullet physics-based position.
        
        Args:
            pos_x: Current X position from PyBullet (meters) - already converted to controller coords
            pos_y: Current Y position from PyBullet (meters) - already converted to controller coords
            heading: Current heading from PyBullet (radians) - already converted to controller coords
            dt: Time step in seconds
            
        Returns:
            Tuple of (left_vel, right_vel) in encoder ticks per sample
        """
        # Update controller state from PyBullet (already converted to controller coordinates)
        ...
        if d < 0.02:  # 2cm tolerance
            if self.waypoints:
                self.current_dest = self.waypoints.pop(0)
                print(f"New destination: ({self.current_dest.pos[0]:.3f}, {self.current_dest.pos[1]:.3f})")
            elif self.original_waypoints:
                # All waypoints completed, reload from original list to loop
                self.waypoints = [WayPoint(wp.pos[0], wp.pos[1], 0.0) for wp in self.original_waypoints]
                self.current_dest = self.waypoints.pop(0)
                print(f"Looping: New destination: ({self.current_dest.pos[0]:.3f}, {self.current_dest.pos[1]:.3f})")
            else:
                # No waypoints at all, stop
                return 0, 0
        
        # Compute wheel velocities using PID control
        # PID controllers have target = 0.0, so error = target - obs = 0 - obs = -obs
        # We pass distance and heading errors directly (not negated) because the coordinate
        # system conversion (negating Y and heading from PyBullet) already accounts for the
        # sign convention needed for correct control direction
        ...
        # Update wheel models to get current velocities (includes motor dynamics)
        ...
        # Return actual velocities from wheel models (includes motor dynamics)
        return lv, rv
    ...
    def reset(self):
        ...
        self.heading = 0.0
        self.pos = np.array([0.0, 0.0])
        # Reload waypoints from original list
        self.waypoints = [WayPoint(wp.pos[0], wp.pos[1], 0.0) for wp in self.original_waypoints]
        if self.waypoints:
            self.current_dest = WayPoint(self.waypoints[0].pos[0], self.waypoints[0].pos[1], 0.0)
        else:
            self.current_dest = WayPoint(0.0, 0.0, 0.0)
